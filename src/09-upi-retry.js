...
export function upiRetry(outcomes) {
  ...
  do{
    ...
    if(attempt < 5){
      totalWaitTime =  totalWaitTime +2 ** (attempt -1);  
    }
  }while(attempt < 5);

  return { attempts: attempt, success: success, totalWaitTime: totalWaitTime }
}

console.log(upiRetry(["fail", "fail", "fail", "fail", "fail"]))